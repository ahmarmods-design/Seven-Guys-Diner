...
// Static fallback images (used when CMS gallery is empty)
...
export function Gallery() {
  ...
  // Use CMS gallery if non-empty, otherwise fall back to static images
  const useCMSImages = gallery.length > 0;
  ...
  return (
    <section id="gallery" className="py-24 bg-[#0A2612] text-white">
      <div className="container mx-auto px-4 md:px-6">
        ...
        {useCMSImages ? (
          /* CMS gallery — simple responsive grid */
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {gallery.map((img, index) => (
              ...
              >
                ...
                <img src={img.url} alt={img.alt} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700 ease-out" loading="lazy" />
              </motion.div>
            ))}
          </div>
        ) : (
          /* Static fallback — original mosaic layout */
          ...
          <Dialog.Content className="fixed left-[50%] top-[50%] z-50 w-full max-w-5xl translate-x-[-50%] translate-y-[-50%] p-4 outline-none">
            ...
            {selectedImg && (
              <div className="relative">
                <img src={selectedImg} alt="Gallery" className="w-full max-h-[80vh] object-contain rounded-2xl" />
                <Dialog.Close className="absolute -top-3 -right-3 w-10 h-10 bg-white text-black rounded-full flex items-center justify-center hover:bg-gray-100 transition-colors shadow-lg">
                  <X size={18} />
                </Dialog.Close>
              </div>
            )}
